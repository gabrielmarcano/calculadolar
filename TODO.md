...
- [x] **Transicion animada continua entre pantallas con reconocimiento de ultima vista**
  - **Descripcion**: Implementar transicion lateral fluida de pantalla completa entre Calculadora y Dashboard activada por botones ("Tasas" y "CALCULADORA") mediante capas independientes deslizables en GPU con aislamiento estricto de eventos táctiles, persistencia de pantalla inicial y maxima prioridad al rendimiento de escritura del teclado numerico.
  - **Alcance**:
    - Sustitucion del carril continuo `w-[200%]` por dos capas independientes superpuestas (`absolute inset-0 w-full h-full`) en `app/page.tsx`, erradicando desfases subpixel y recortes en el borde derecho de la calculadora.
    - Transicion lateral sincronizada con `translate3d` por hardware y curva `cubic-bezier(0.16, 1, 0.3, 1)`, desactivando animaciones espurias en el montaje inicial (`hasMounted`).
    - Aislamiento bidireccional de eventos táctiles (`pointer-events-none` en la pantalla inactiva y `pointer-events-auto` en la activa) impidiendo bloqueos o intercepciones fantasmas.
    - Ejecucion inmediata en `pointerdown` con `e.preventDefault()` y vibracion háptica en el boton "Tasas", selector USD/VES y boton flotante "CALCULADORA", garantizando respuesta instantanea (0 ms) sin cancelacion por deslizamiento del pulgar.
    - Extraccion atomica de `components/CalculatorTopBar.tsx` (<140 lineas) y compactacion de `components/CalculatorView.tsx` (<120 lineas) en estricto cumplimiento del estandar de arquitectura limpia (<200 lineas).
